...
@pytest.mark.asyncio
async def test_run_code_python_context_persistence(code_session):
...
""".strip()
    ...
    setup_result = await code_session.run_code(setup_code, "python")
...
""".strip()
    ...
    use_result = await code_session.run_code(use_code, "python")
...
@pytest.mark.asyncio
async def test_js_error_handling(code_session):
