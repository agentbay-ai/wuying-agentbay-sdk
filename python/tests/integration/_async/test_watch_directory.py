...
@pytest.mark.asyncio
async def test_watch_directory(make_session):
    """
    ...
    try:
        ...
        write_result = await session.file_system.write_file(test_file1, "Hello, World!")
        assert write_result.success, f"Failed to write file: {write_result.error_message}"
        await asyncio.sleep(3)

        print("\n4. Modifying the file...")
        modify_result = await session.file_system.write_file(test_file1, "Modified content")
...
@pytest.mark.asyncio
async def test_watch_directory_no_events_after_stop(make_session):
    """
    ...
    await session.file_system.write_file(f"{test_dir}/before_stop.txt", "before stop")
    ...
    await session.file_system.write_file(f"{test_dir}/after_stop.txt", "after stop")
    await asyncio.sleep(3)
