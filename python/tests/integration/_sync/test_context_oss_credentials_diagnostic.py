...
def _create_diagnostic_contexts(agent_bay_client, unique: str):
    created_contexts = []
    for suffix in ("workspace", "shared"):
        context_name = f"smartclaw-credential-diagnostic-{suffix}-{unique}"
        context_result = agent_bay_client.context.create(context_name)
...
def _wait_for_target_download_statuses(session, context_ids: set[str]):
    ...
    while time.monotonic() < deadline:
        latest_info = session.context.info()
        latest_statuses = [
            item
...
@pytest.mark.sync
def test_smartclaw_workspace_contexts_receive_download_credentials(
    agent_bay_client,
):
    ...
    try:
        created_contexts = _create_diagnostic_contexts(agent_bay_client, unique)
        ...
        info_result, statuses = _wait_for_target_download_statuses(
            session,
            target_context_ids,
        )
...
@pytest.mark.sync
def test_load_skills_with_smartclaw_contexts_does_not_starve_credentials(
    agent_bay_client,
):
...
@pytest.mark.sync
def test_duplicate_bind_during_initial_download_does_not_starve_credentials(
    agent_bay_client,
):
