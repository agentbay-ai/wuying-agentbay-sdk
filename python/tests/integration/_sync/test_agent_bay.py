...
class TestWhiteListPattern(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        if self.session:
            try:
                ...
                print(f"Warning: Error deleting session: {e}")
        if self.context:
            context_result = self.agent_bay.context.delete(self.context)
            self.assertTrue(context_result.success)
            if context_result.success:
                print(f"Deleted context {self.context.id}")

    def _collect_all_files(self, context_id: str, folder_path: str) -> list:
        ...
        """
        result = self.agent_bay.context.list_files(
        ...
        for entry in result.entries:
            ...
            if ftype in ("FOLDER", "DIR", "DIRECTORY"):
                ...
                file_names.extend(
                    self._collect_all_files(context_id, sub_path)
                )
            else:
                # FILE: store the file name
                file_names.append(last_segment)
        return file_names

    @pytest.mark.sync
    def test_create_session_with_pattern_bwlist(self):
        ...
        Single-session strategy:
          ...
          4. list_files(base) to get sub-directory entries.
          5. Recursively collect all FILE entries using last_segment of entry.file_path.
        ...
        context_sync = ContextSync(context_id, path=base, policy=sync_policy)
        params = CreateSessionParams(
        ...
        del_result = self.agent_bay.delete(self.session, sync_context=True)
        ...
        probe = self.agent_bay.context.list_files(
        ...
        if probe.entries:
            for e in probe.entries:
                ...
                if ftype in ("FOLDER", "DIR", "DIRECTORY"):
                    ...
                    sub_files = self._collect_all_files(context_id, sub_path)
