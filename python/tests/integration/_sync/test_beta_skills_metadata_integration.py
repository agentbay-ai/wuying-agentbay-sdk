...
# agent_bay_client fixture (AgentBay, scope="module") is provided by conftest.py
...
@pytest.mark.sync
def test_beta_skills_get_metadata(agent_bay_client: AgentBay):
    """Verify async get_metadata returns a valid SkillsMetadataResult with expected fields."""
    result = agent_bay_client.beta.skills.get_metadata()
    assert isinstance(result, SkillsMetadataResult)
    assert len(result.skills) > 0
...
@pytest.mark.sync
def test_beta_skills_get_metadata_sync_async_parity(
    agent_bay_client: AgentBay, agent_bay_sync: AgentBay
):
    """Verify that async and sync SDKs return identical skill metadata."""
    async_result = agent_bay_client.beta.skills.get_metadata()
