...
@pytest.mark.sync
def test_beta_skills_list_metadata(agent_bay_client: AgentBay):
    """Verify async list_metadata returns a valid, non-empty list with expected fields."""
    items = agent_bay_client.beta.skills.list_metadata()
    assert isinstance(items, list)
    assert len(items) > 0
    first = items[0]
...
def test_beta_skills_list_metadata_sync_async_parity(
    ...
    async_items = agent_bay_client.beta.skills.list_metadata()
