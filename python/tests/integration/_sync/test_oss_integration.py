...
@pytest.mark.sync
def test_env_init(make_session):
...
def test_upload(make_session):
    ...
    result = session.oss.upload(bucket, object_key, test_file_path)
...
@pytest.mark.sync
def test_upload_anonymous(make_session):
    ...
    result = session.oss.upload_anonymous(upload_url, test_file_path)
...
@pytest.mark.sync
def test_download(make_session):
    ...
    result = session.oss.download(bucket, object_key, download_path)

    assert result.success
    assert result.request_id is not None
    assert "Download success" in result.content

    file_info = session.file_system.get_file_info(download_path)
    assert file_info is not None


@pytest.mark.sync
def test_download_anonymous(make_session):
    ...
    result = session.oss.download_anonymous(download_url, download_path)
