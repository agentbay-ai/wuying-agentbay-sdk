...
@pytest.mark.sync
def test_start_and_stop_app(session):
    ...
    try:
        ...
        start_result = session.mobile.start_app(start_cmd)
        ...
        stop_result = session.mobile.stop_app_by_cmd(stop_cmd)
...
@pytest.mark.sync
def test_get_clickable_ui_elements(session):
    """Test retrieving clickable UI elements."""
    try:
        result = session.mobile.get_clickable_ui_elements(timeout_ms=10000)
...
@pytest.mark.sync
def test_get_all_ui_elements(session):
    """Test retrieving all UI elements."""
    try:
        result = session.mobile.get_all_ui_elements(timeout_ms=10000)
        assert isinstance(result, UIElementListResult)
