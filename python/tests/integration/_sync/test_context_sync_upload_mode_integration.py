...
@pytest.mark.sync
def test_create_session_with_default_file_upload_mode(agent_bay_client, unique_id):
    ...
    context_result = agent_bay_client.context.get(context_name, True)
    ...
    delete_result = agent_bay_client.delete(session, sync_context=True)
...
@pytest.mark.sync
def test_context_sync_with_archive_mode_and_file_operations(agent_bay_client, unique_id):
    ...
    direction_result =  session.file_system.create_directory("/tmp/archive-mode-test")
    ...
    list_result = agent_bay_client.context.list_files(
...
@pytest.mark.sync
def test_invalid_upload_mode_with_policy_assignment(agent_bay_client, unique_id):
...
@pytest.mark.sync
def test_valid_upload_mode_values(agent_bay_client, unique_id):
...
@pytest.mark.sync
def test_archive_mode_with_exclude_paths(agent_bay_client, unique_id):
    ...
    try:
        ...
        dir_result = session.file_system.create_directory(
        ...
        if session is not None:
            agent_bay_client.delete(session, sync_context=True)
