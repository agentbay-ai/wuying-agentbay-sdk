...
def collect_all_files(agent_bay, context_id: str, folder_path: str) -> list:
    ...
    for entry in result.entries:
        ...
        if ftype in ("FOLDER", "DIR", "DIRECTORY"):
            ...
            file_paths.extend(
                collect_all_files(agent_bay, context_id, sub_path)
            )
        else:
...
def example_1_recycle_policy_one_day():
    ...
    try:
        ...
        try:
            ...
            mkdir_result = session.file_system.create_directory("/tmp/oneday_data")
...
def example_2_archive_mode_with_exclude_paths():
    ...
    try:
        ...
        for d in [f"{sync_path}/important", f"{sync_path}/regular"]:
            dir_result = session.file_system.create_directory(d)
        ...
        for file_path, content in test_files:
            write_result = session.file_system.write_file(file_path, content, mode="overwrite")
...
def example_3_bwlist_with_regex():
    ...
    try:
        ...
        all_paths = collect_all_files(agent_bay, context_id, base)
...
def main():
    ...
    try:
        example_1_recycle_policy_one_day()
        example_2_archive_mode_with_exclude_paths()
        example_3_bwlist_with_regex()
