...
def download_file_from_context(
    agentbay: AgentBay,
    context_id: str,
    file_path: str,
    download_dir: str
    ...
    """
    try:
        url_result = agentbay.context.get_file_download_url(context_id, file_path)
        if not url_result.success:
            print(f"  ✗ Failed to get download URL for {file_path}: {url_result.error_message}")
            return None

        with httpx.Client() as client:
            response = client.get(url_result.url, timeout=30.0)
...
def file_operations_and_lifecycle_demonstration():
    ...
    try:
        ...
        for folder_path in files_check:
            time.sleep(3)
            list_result = agentbay.context.list_files(context_id, folder_path)
        ...
        for file_path in uploaded_content:
            local_path = download_file_from_context(agentbay, context_id, file_path, download_dir)
            if local_path:
        ...
        print("\n  Test 1: Invalid context ID")
        try:
            result = agentbay.context.get_file_upload_url("invalid-context-id", "/test.txt")
        ...
        print("\n  Test 2: Non-existent file download")
        try:
            result = agentbay.context.get_file_download_url(context_id, "/nonexistent.txt")
            if not result.success:
        ...
        print("\n  Test 3: Delete non-existent file")
        try:
            delete_result = agentbay.context.delete_file(context_id, "/nonexistent.txt")
...
def main():
    ...
    file_operations_and_lifecycle_demonstration()
