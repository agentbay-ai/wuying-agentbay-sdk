...
def main():
    ...
    try:
        session_result = client.create(CreateSessionParams())
        ...
        handle = session.pty.create(on_data=on_data)
        print(f"1. Created PTY: {handle.pty_session_id}")
        time.sleep(1)
        handle.send_input(b"echo 'AGENTBAY_PTY_EXAMPLE_ECHO'\r")
        time.sleep(2)
        ...
        handle2 = session.pty.connect(pty_id, on_data=on_data2)
        time.sleep(1)
        handle2.send_input(b"echo 'AGENTBAY_PTY_EXAMPLE_RECONNECT'\r")
        time.sleep(2)
        ...
        # 5. Kill (separate PTY)
        kill_h = session.pty.create()
        time.sleep(1)
        kill_h.kill()
        kill_code = kill_h.wait(timeout_s=10)
        print(f"5. Kill exit code: {kill_code} (expected -9)\n")

        # 6. Exit with wait
        exit_h = session.pty.create()
        time.sleep(1)
        exit_h.send_input(b"exit\r")
        exit_code = exit_h.wait(timeout_s=10)
