...
describe("LifecyclePolicy integration", () => {
  ...
  it(
    "should create session with custom lifecycle policy",
    async () => {
      ...
      try {
        const cmdResult =
          await session.command.executeCommand("echo hello");
  ...
  it(
    "should create session with manual release",
    async () => {
      const agentBay = new AgentBay({ apiKey: getTestApiKey() });
      ...
      try {
        const cmdResult =
          await session.command.executeCommand("echo manual");
  ...
  it(
    "should create session with default lifecycle policy",
    async () => {
      const agentBay = new AgentBay({ apiKey: getTestApiKey() });
      const params: CreateSessionParams = {
        imageId,
